...
- **Join-key shape (resolved 2026-08-02)**: scoped to `tool_result` content blocks only for v1 — matches both worked examples above (re-read file, polled API response) and avoids the harder, unresolved problem of identifying "the same" plain prose text recurring across turns without an id (that's near_dup's job, not this stage's). Keyed by `tool_use_id`: `config["previous_payloads"]: dict[str, str]` maps a tool_result's `tool_use_id` (unique within the current request, already known to the caller when they construct that reply) to whatever they consider its previous version — no new wire fields, no caller-side id-tracking scheme to invent. Format picks JSON patch when the content parses as JSON (the "polled API response" case) or a unified diff otherwise (the "re-read file" case). Flagged, not resolved here: whether Claude reliably interprets a diff/patch as "a delta against something you saw earlier" is a real quality question, not just a mechanical one — belongs in Phase 9's held-out quality-check set, same treatment as brevity/thinking-effort, rather than assumed to just work.
...
- **2026-08-02** — Resolved stage 6 (delta compression)'s join-key question during Phase 7 planning, before implementation started: the brief already established *that* the previous payload is caller-supplied (external, cheap, no-persistence, matching stages 7/8's shapes), but not *how* a pipeline stage matches a caller-supplied previous payload to the right content block in the current request. Resolved by scoping to `tool_result` blocks only (matches both of the brief's worked examples; general prose-text repeats stay near_dup's job) and keying `config["previous_payloads"]` off each block's `tool_use_id`, which the caller already has in hand when constructing that reply. Also flagged, not resolved: whether Claude actually interprets a diff/patch correctly is a quality question for Phase 9's held-out quality-check set, not a mechanical one to assume away. See the amended stage 6 section above for full detail.

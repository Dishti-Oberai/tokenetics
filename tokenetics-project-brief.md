...
- **Mechanism — max_tokens/stop-sequences**: derive an expected output size from the task type (short factual → small; code/multi-step → large; extraction → sized to schema), set `max_tokens` = estimate + 15-20% safety margin. Monitor actual truncation events (responses that hit the cap) via the logger; if a task type's truncation rate exceeds ~1-2% over a rolling window, auto-widen that type's margin and log the adjustment. Stop-sequences set only where a natural terminator is known safe (never inside code-heavy tasks).
